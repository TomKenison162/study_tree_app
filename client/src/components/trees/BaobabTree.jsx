...
const BRANCHES = [
  { d:"M260,230 C230,210 190,195 140,180", w:11, da:135, delay:0    },
  { d:"M350,228 C380,208 420,194 470,178", w:11, da:140, delay:0.03 },
  { d:"M255,250 C225,238 185,230 130,228", w:8,  da:135, delay:0.06 },
  { d:"M355,248 C385,236 425,228 480,226", w:8,  da:140, delay:0.08 },
  { d:"M272,215 C258,198 240,178 218,158", w:9,  da:70,  delay:0.04 },
  { d:"M338,213 C352,196 370,176 392,156", w:9,  da:70,  delay:0.05 },
  { d:"M290,195 C280,172 268,148 258,120", w:7,  da:85,  delay:0.10 },
  { d:"M318,194 C328,170 340,146 350,118", w:7,  da:85,  delay:0.12 },
];

const CROWNS = [
  { cx:140, cy:170, rx:35, ry:25, swayDur:5.5, pRange:[0.35,0.62] },
  { cx:470, cy:168, rx:35, ry:25, swayDur:4.8, pRange:[0.37,0.64] },
  { cx:130, cy:220, rx:30, ry:20, swayDur:6.0, pRange:[0.33,0.60] },
  { cx:480, cy:218, rx:30, ry:20, swayDur:5.2, pRange:[0.36,0.63] },
  { cx:218, cy:148, rx:30, ry:22, swayDur:4.6, pRange:[0.39,0.66] },
  { cx:392, cy:146, rx:30, ry:22, swayDur:5.8, pRange:[0.41,0.68] },
  { cx:258, cy:110, rx:28, ry:20, swayDur:5.1, pRange:[0.43,0.70] },
  { cx:350, cy:108, rx:28, ry:20, swayDur:4.9, pRange:[0.45,0.72] },
  { cx:300, cy:150, rx:24, ry:18, swayDur:5.4, pRange:[0.32,0.58] },
];

export default function BaobabTree({ progress, theme, sunAngleNorm, sunHeight, duskOp, rimLightOp, nightOp }) {
  const trP  = c01(progress, 0.00, 0.22);
  const detP = c01(progress, 0.50, 0.80);

  const trunkSX = 0.65 + trP * 0.35;
  ...
  return (
    <>
      ...
      <path d="M262,524 C232,514 190,518 140,534" stroke="#6a5040" strokeWidth="15" strokeLinecap="round" fill="none"
        strokeDasharray="135" strokeDashoffset={135*(1-trP)} style={{ transition:dashTr }}/>
      ...
      <path d="M275,528 C252,536 220,545 185,554" stroke="#6a5040" strokeWidth="9" strokeLinecap="round" fill="none"
        strokeDasharray="95" strokeDashoffset={95*(1-trP)} style={{ transition:dashTr }}/>
      ...
      <path d={TRUNK_D} fill="#6a5040"
        style={{ transform:`scaleY(${trP}) scaleX(${trunkSX})`, transformOrigin:'300px 524px', transition:'transform 2.2s cubic-bezier(0.34,1.1,0.64,1)' }}/>
      ...
      <path d="M285,524 C283,450 282,380 284,320 C286,275 292,245 300,225 C308,245 314,275 316,320 C318,380 317,450 315,524"
        ...
        style={{ transform:`scaleY(${trP}) scaleX(${trunkSX})`, transformOrigin:'300px 524px', transition:'transform 2.2s' }}/>
      ...
      {BRANCHES.map((b, i) => {
        const brP = c01(progress, 0.16 + b.delay, 0.40 + b.delay);
        return (
          <g key={`bb${i}`} style={{
            animation: brP > 0.6 ? `baobabCreak ${7+i*0.8}s ease-in-out ${i*0.6}s infinite` : 'none',
            transformOrigin: i < 4 ? '300px 240px' : '300px 200px',
          }}>
            <path d={b.d} stroke="#5a4030" strokeWidth={b.w} strokeLinecap="round" fill="none"
              strokeDasharray={b.da} strokeDashoffset={b.da*(1-brP)} style={{ transition:dashTr }}/>
          </g>
        );
      })}

      {CROWNS.map((cr, i) => {
        const lfP = c01(progress, cr.pRange[0], cr.pRange[1]);
        return (
          <g key={`bc${i}`} style={{
            animation: lfP > 0.6 ? `canopySway ${cr.swayDur}s ease-in-out ${i*0.45}s infinite` : 'none',
            transformOrigin: `${cr.cx}px ${cr.cy}px`,
          }}>
            <g style={{
              transform: `scale(${lfP}) translateY(${(1 - lfP) * 5}px)`,
              transformOrigin: `${cr.cx}px ${cr.cy}px`,
              transition: 'transform 1.6s cubic-bezier(0.22,1.6,0.36,1), opacity 0.8s ease-out',
              opacity: Math.min(1, lfP * 3),
            }}>
      ...
      {rimLightOp > 0.1 && (
        <path d={TRUNK_D} fill={duskOp>0.5?'#c89060':'#ffe8c0'} opacity={rimLightOp*0.15}
          style={{
            transform:`scaleY(${trP}) scaleX(${trunkSX}) translateX(${sunAngleNorm*8}px)`,
