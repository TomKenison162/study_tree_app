...
const FRONDS = [
  { angle:-80, len:140, curve:50,  pKey:'fr'  },
  { angle:-55, len:155, curve:65,  pKey:'fr'  },
  { angle:-30, len:165, curve:80,  pKey:'fr'  },
  { angle:-10, len:150, curve:90,  pKey:'fr2' },
  { angle:15,  len:160, curve:85,  pKey:'fr2' },
  { angle:40,  len:155, curve:70,  pKey:'fr2' },
  { angle:65,  len:140, curve:55,  pKey:'fr2' },
  { angle:85,  len:120, curve:45,  pKey:'fr'  },
  { angle:-95, len:110, curve:40,  pKey:'fr2' },
  { angle:100, len:105, curve:38,  pKey:'fr2' },
];

/* Per-frond sway durations for organic wind */
...
export default function PalmTree({ progress, theme, sunAngleNorm, sunHeight, duskOp, rimLightOp, nightOp }) {
  ...
  const frP  = c01(progress, 0.20, 0.55);
  const fr2P = c01(progress, 0.35, 0.65);
  const detP = c01(progress, 0.50, 0.80);

  const pMap = { fr: frP, fr2: fr2P };
  ...
  return (
    <>
      ...
      <path d="M294,524 C282,518 268,522 252,530" stroke="#5a4020" strokeWidth="8" strokeLinecap="round" fill="none"
        strokeDasharray="48" strokeDashoffset={48*(1-trP)} style={{ transition:TR }}/>
      <path d="M308,524 C320,518 334,522 350,530" stroke="#5a4020" strokeWidth="8" strokeLinecap="round" fill="none"
        strokeDasharray="48" strokeDashoffset={48*(1-trP)} style={{ transition:TR }}/>

      {/* Trunk — slightly curved palm */}
      <path d={trunkPath}
        stroke="#3a1e0a" strokeWidth="10" strokeLinecap="round" fill="none" opacity="0.15"
        strokeDasharray="440" strokeDashoffset={440*(1-trP)}
        style={{ transition:'stroke-dashoffset 2.2s cubic-bezier(0.34,1.2,0.64,1)' }}/>
      <path d={trunkPath}
        stroke="#6a4820" strokeWidth="28" strokeLinecap="round" fill="none"
        strokeDasharray="440" strokeDashoffset={440*(1-trP)}
        style={{ transition:'stroke-dashoffset 2.2s cubic-bezier(0.34,1.2,0.64,1)' }}/>
      <path d={`M302,520 C300,460 304,400 298,340 C294,280 300,220 304,160 C306,132 310,117 ${CROWN_X+2},${CROWN_Y+2}`}
        stroke="#8a6840" strokeWidth="10" strokeLinecap="round" fill="none" opacity="0.4"
        strokeDasharray="440" strokeDashoffset={440*(1-trP)}
        style={{ transition:'stroke-dashoffset 2.2s cubic-bezier(0.34,1.2,0.64,1)' }}/>
      ...
      {/* Fronds with independent flex animation */}
      {FRONDS.map((f, i) => {
        const p = pMap[f.pKey];
