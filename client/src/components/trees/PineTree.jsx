...
const TIERS = [
  { apex:88,  base:172, hw:44,  pRange:[0.06,0.20] },
  { apex:128, base:220, hw:60,  pRange:[0.14,0.29] },
  { apex:174, base:276, hw:78,  pRange:[0.22,0.38] },
  { apex:226, base:338, hw:98,  pRange:[0.31,0.47] },
  { apex:284, base:404, hw:119, pRange:[0.40,0.57] },
  { apex:350, base:472, hw:142, pRange:[0.50,0.67] },
  { apex:420, base:524, hw:164, pRange:[0.60,0.77] },
];
...
/* Staggered durations so each tier sways at its own pace */
  ...
  return (
    <>
      ...
      <path d="M294,520 C272,508 250,518 230,528" stroke="#3a1808" strokeWidth="9" strokeLinecap="round" fill="none"
        strokeDasharray="72" strokeDashoffset={72*(1-trP)} style={{ transition:TR }}/>
      <path d="M306,522 C330,510 354,520 372,528" stroke="#3a1808" strokeWidth="9" strokeLinecap="round" fill="none"
        strokeDasharray="72" strokeDashoffset={72*(1-trP)} style={{ transition:TR }}/>

      {/* Trunk — multi-layer for bark depth */}
      <path d="M296,524 C296,460 297,350 298,200 C298,150 299,110 299,88"
        stroke="#1a0a04" strokeWidth="16" strokeLinecap="round" fill="none" opacity="0.15"
      ...
      {TIERS.map(({ apex, base, hw }, i) => {
        ...
        const swayAnim = p > 0.5
          ? `canopySway ${SWAY_DURATIONS[i]}s ease-in-out ${i*0.35}s infinite`
          : 'none';
        const shiverAnim = p > 0.7
          ? `pineTierShiver ${SHIVER_DURATIONS[i]}s ease-in-out ${i*0.22}s infinite`
          : 'none';
        return (
          <g key={i}
            style={{
            ...
            {/* Shiver group — separated so it layers on top */}
            {p > 0.7 && (
            ...
            {/* Snow on upper tiers at high progress */}
            {progress > 0.88 && i < 2 && (
      ...
      {/* Crown tip star — spins slowly when fully grown */}
      {detP > 0.6 && (
