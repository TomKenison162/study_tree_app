...
const CHAINS = [
  { x:130, y:268, len:150, d:0,   pKey:'ch1' },
  { x:155, y:252, len:175, d:0.3, pKey:'ch1' },
  { x:178, y:240, len:185, d:0.6, pKey:'ch1' },
  { x:200, y:233, len:170, d:0.2, pKey:'ch1' },
  { x:222, y:228, len:155, d:0.8, pKey:'ch2' },
  { x:245, y:235, len:140, d:0.1, pKey:'ch2' },
  { x:268, y:232, len:160, d:0.5, pKey:'ch2' },
  { x:290, y:230, len:170, d:0.7, pKey:'ch2' },
  { x:310, y:230, len:168, d:0.2, pKey:'ch2' },
  { x:332, y:232, len:155, d:0.4, pKey:'ch2' },
  { x:355, y:235, len:145, d:0.9, pKey:'ch3' },
  { x:378, y:240, len:160, d:0.3, pKey:'ch3' },
  { x:400, y:248, len:180, d:0.6, pKey:'ch3' },
  { x:422, y:255, len:175, d:0.1, pKey:'ch3' },
  { x:445, y:265, len:155, d:0.5, pKey:'ch3' },
  { x:465, y:275, len:135, d:0.8, pKey:'ch3' },
  { x:235, y:248, len:120, d:0.4, pKey:'ch2' },
  { x:365, y:248, len:125, d:0.7, pKey:'ch3' },
];

/* Different sway durations per chain for more organic motion */
const SWAY_DURATIONS = [5.2,4.8,5.5,4.6,5.0,5.8,4.7,5.3,4.9,5.6,5.1,4.8,5.4,5.0,4.7,5.2,5.5,4.9];
/* Chain tip bob durations */
...
export default function WisteriaTree({ progress, theme, sunAngleNorm, sunHeight, duskOp, rimLightOp, nightOp }) {
  ...
  const pMap = { ch1: ch1P, ch2: ch2P, ch3: ch3P };
  ...
  return (
    <>
      ...
      {CHAINS.map((ch, i) => {
        const p = pMap[ch.pKey];
        const n = Math.floor(ch.len / 16);
        return (
          <g key={`wch${i}`}
            style={{
              transform:`scaleY(${p})`, transformOrigin:`${ch.x}px ${ch.y}px`,
            ...
            {/* Chain tip bob — bottom blossoms droop expressively */}
            {p > 0.6 && (
